...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
		...
		wp_enqueue_script('jquery-maskedinput', get_template_directory_uri().'/js/jquery.maskedinput.min.js','','',true);
		wp_enqueue_script('main', get_template_directory_uri().'/js/scripts.js','', '2.' . time(), true);
		wp_enqueue_script('magnific-popup', get_template_directory_uri().'/js/magnific-popup/jquery.magnific-popup.min.js','','',true);
	}
}
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
	    wp_enqueue_style( 'normalize', get_template_directory_uri().'/css/libs/normalize.css' ); //
	    wp_enqueue_style( 'bootstrap-grid', get_template_directory_uri().'/css/libs/bootstrap-grid.css' ); //
	    wp_enqueue_style( 'magnific-popup', get_template_directory_uri().'/js/magnific-popup/magnific-popup.css' ); //
		wp_enqueue_style( 'main', get_template_directory_uri().'/css/main.css', array(), '2.' . time() ); // основные стили шаблона
		wp_enqueue_style( 'style', get_template_directory_uri().'/style.css', array(), '2.' . time() );
	}
}

add_action('init', 'smartdevice_custom_init');
function smartdevice_custom_init(){
    register_post_type('products', array(
        'labels'             => array(
            ...
            'search_items'       => 'Найти продукцию',
            'not_found'          =>  'Продукции не найдено',
            'not_found_in_trash' => 'В корзине продукции не найдено',
            'parent_item_colon'  => '',
            'menu_name'          => 'Продукция'
...
function smart_get_format_cost($price){
